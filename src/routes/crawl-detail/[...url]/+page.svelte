<script lang="ts">
	...
	let sampleQueries = [
		{
			name: 'Broken Links',
			query: "SELECT href, text, status_state FROM links WHERE status_state = 'broken'"
		},
		{ name: 'Images Missing Alt', query: 'SELECT src, alt FROM images WHERE is_missing = 1' },
		{
			name: 'H1 & H2 Headings',
			query: "SELECT tag, text FROM headings WHERE tag = 'h1' OR tag = 'h2'"
		},
		{
			name: 'Meta Warning Issues',
			query: "SELECT name, content, status FROM meta WHERE status = 'warning' OR status = 'missing'"
		}
	];
	...
	const summary = $derived(
		auditResults ? summarizeAudit(auditResults) : { errorCount: 0, warningCount: 0, passedCount: 0 }
	);
		...
		<section class="scanning-wrapper container mt-4">
			<div class="scanning-header font-mono text-center">
				...
				<p class="text-muted text-xs mt-1">
					Analyzing client-side DOM structure, checking robots schema, and parsing outbound
					hyper-references.
				</p>
	...
	{:else if crawlError && !auditResults}
		...
		<section class="error-wrapper container mt-4">
			<div class="error-header font-mono text-center">
				...
				<p class="text-muted text-xs mt-1">
					The audit could not be completed. Review the error and the captured log stream below, then
					retry.
				</p>
				<p class="monotext text-ink text-sm mt-2">{currentUrl}</p>
			</div>
			...
			<div class="container">
						...
						<div class="grade-info">
							...
							<div class="flex-row mt-1">
								<span class="scan-time"
									>Checked: {new Date(auditResults.timestamp).toLocaleTimeString()}</span
								>
				...
				<div class="action-banner card-dark">
					<div class="banner-text">
						...
						<p>
							Generate a technical PDF report containing all heading outlines, broken link details,
							and PageSpeed metrics.
						</p>
					</div>
				...
				<div class="tabs-nav">
					...
					<button
						...
						onclick={() => (activeTab = 'ai-chat')}>✨ AI Chat</button
					>
					<button
						class="tab-btn"
						class:active={activeTab === 'onpage'}
						onclick={() => (activeTab = 'onpage')}>On-Page SEO</button
					>
					<button
						class="tab-btn"
						class:active={activeTab === 'links'}
						onclick={() => (activeTab = 'links')}>Links Auditor</button
					>
					<button
						class="tab-btn"
						class:active={activeTab === 'pagespeed'}
						onclick={() => (activeTab = 'pagespeed')}>PageSpeed</button
					>
					<button
						class="tab-btn sql-tab-btn"
						class:active={activeTab === 'sql'}
						onclick={() => (activeTab = 'sql')}>SQL Console</button
					>
				</div>
				...
				{#if activeTab === 'overview'}
					<div class="tab-content" id="overview-content">
						<DashboardTab {auditResults} {jumpToSection} />
					</div>
				{/if}
				...
				{#if activeTab === 'links'}
					<div class="tab-content" id="links-content">
						<LinksTab {auditResults} {isValidatingLinks} {checkedLinksCount} {totalLinksCount} />
					</div>
				{/if}
				...
				{#if activeTab === 'sql'}
					<div class="tab-content" id="sql-content">
						<SqlConsoleTab {auditResults} bind:sqlQuery {sqlResult} {sqlError} {runSQLQuery} />
					</div>
				{/if}
