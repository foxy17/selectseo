...
function prepareDatabase(results: AuditResults): Database {
  ...
  `);
	...
	const metaStmt = db.prepare(
		`INSERT INTO meta (name, content, length, status, message) VALUES (?, ?, ?, ?, ?)`
	);
  ...
  `);
	...
	const linksStmt = db.prepare(
		`INSERT INTO links (id, href, text, type, secure, status, status_state, rel) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
	);
