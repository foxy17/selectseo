...
export function exportSEOReport(results: AuditResults): void {
	...
	if (results.score >= 90)
		...
		doc.setTextColor(245, 158, 11); // Warning Yellow/Orange
	...
	doc.setFontSize(14);
	doc.setTextColor(
		brokenLinksCount > 0 ? 239 : 20,
		brokenLinksCount > 0 ? 68 : 20,
		brokenLinksCount > 0 ? 68 : 20
	);
	...
	const onPageItems = [
		{
			name: 'Title Tag',
			status: results.onPage.title.status,
			detail: results.onPage.title.text,
			msg: results.onPage.title.message
		},
		{
			name: 'Meta Description',
			status: results.onPage.description.status,
			detail: results.onPage.description.text,
			msg: results.onPage.description.message
		},
		{
			name: 'Viewport Meta',
			status: results.onPage.viewportAudit.status,
			detail: results.onPage.viewport || 'Missing',
			msg: results.onPage.viewportAudit.message
		},
		{
			name: 'HTML Language',
			status: results.onPage.languageAudit.status,
			detail: results.onPage.contentMetrics.language,
			msg: results.onPage.languageAudit.message
		},
		{
			name: 'Robots Directive',
			status: results.onPage.robotsMetaAudit.status,
			detail: results.onPage.robots,
			msg: results.onPage.robotsMetaAudit.message
		},
		{
			name: 'Favicon Presence',
			status: results.onPage.faviconAudit.status,
			detail: results.onPage.favicon || 'Missing',
			msg: results.onPage.faviconAudit.message
		},
		{
			name: 'Canonical Link',
			status: results.onPage.canonical.status,
			detail: results.onPage.canonical.url,
			msg: results.onPage.canonical.message
		},
		{
			name: 'Header H1 Tag',
			status: results.onPage.headings.status,
			detail: `Found ${results.onPage.headings.h1.length} H1 headers.`,
			msg: results.onPage.headings.message
		},
		{
			name: 'Image Alt Tags',
			status: results.onPage.imageAlts.status,
			detail: `Missing ${results.onPage.imageAlts.missing} out of ${results.onPage.imageAlts.total} alts.`,
			msg: results.onPage.imageAlts.message
		}
	];
	...
	onPageItems.forEach((item) => {
		...
		if (item.detail) {
			...
			const detailLines = doc.splitTextToSize(
				`Content: "${item.detail.slice(0, 120)}${item.detail.length > 120 ? '...' : ''}"`,
				contentWidth - 40
			);
	...
	if (results.pageSpeedMobile || results.pageSpeedDesktop) {
		...
		const renderDeviceStats = (title: string, metrics: PageSpeedMetric) => {
			...
			doc.setFont('Helvetica', 'bold');
			doc.setTextColor(
				metrics.score >= 90 ? 34 : metrics.score >= 50 ? 245 : 239,
				metrics.score >= 90 ? 197 : metrics.score >= 50 ? 158 : 68,
				metrics.score >= 90 ? 94 : metrics.score >= 50 ? 11 : 68
			);
			...
			doc.setTextColor(80, 80, 80);
			doc.text(
				`LCP: ${metrics.lcp}   |   TBT: ${metrics.tbt}   |   CLS: ${metrics.cls}`,
				margin + 70,
				y + 9
			);
			y += 18;
			...
			if (metrics.recommendations.length > 0) {
				...
				metrics.recommendations.forEach(
					(rec: { title: string; description: string; displayValue?: string }) => {
						doc.setFont('Helvetica', 'bold');
						doc.setFontSize(8.5);
	...
	for (let i = 1; i <= totalPagesCount; i++) {
		...
		doc.text(
			'Report powered by SelectSEO Auditor - Client-side Analytics Engine',
			margin,
			pageHeight - 10
		);
	}
