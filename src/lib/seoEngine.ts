...
export async function runSEOAudit(
	...
	let robotsTxtAi: GenericAudit = {
		status: 'ok',
		message: 'No AI bots explicitly blocked in robots.txt.'
	};
	try {
		...
		if (robotsRes.ok) {
			...
			if (
				lowerRobots.includes('user-agent: gptbot') ||
			...
			robotsTxtAi = {
				status: 'ok',
				message: 'robots.txt not found or unreachable, assuming AI bots are allowed.'
			};
		}
	} catch {
		robotsTxtAi = {
	...
	if (descLength === 0) {
		descStatus = 'missing';
		descMessage =
			'Meta description tag is missing. Search engines may use random snippets instead.';
	...
	const canonicalAudit: CanonicalAudit = {
		url: canonicalUrl,
	...
	const h1Elements = Array.from(doc.querySelectorAll('h1')).map(
		(el) => el.textContent?.trim() || ''
	);
	const h2Elements = Array.from(doc.querySelectorAll('h2')).map(
		(el) => el.textContent?.trim() || ''
	);
	const h3Elements = Array.from(doc.querySelectorAll('h3')).map(
		(el) => el.textContent?.trim() || ''
	);
	const h4Elements = Array.from(doc.querySelectorAll('h4')).map(
		(el) => el.textContent?.trim() || ''
	);
	const h5Elements = Array.from(doc.querySelectorAll('h5')).map(
		(el) => el.textContent?.trim() || ''
	);
	const h6Elements = Array.from(doc.querySelectorAll('h6')).map(
		(el) => el.textContent?.trim() || ''
	);
	...
	if (h1Elements.length === 0) {
		...
		headingsMessage =
			'Missing H1 heading. Every page needs exactly one H1 to state its primary topic.';
	...
	const ogDesc =
		doc
			.querySelector('meta[property="og:description"], meta[name="og:description"]')
			?.getAttribute('content') || '';
	const ogImage =
		doc
			...
			?.getAttribute('content') || '';
	const ogType =
		doc.querySelector('meta[property="og:type"], meta[name="og:type"]')?.getAttribute('content') ||
		'';
	...
	const twitterStatus: TwitterCardAudit['status'] =
		twitterCardTag && twitterTitle && twitterDesc ? 'ok' : 'missing';
	...
	const hreflangs = Array.from(doc.querySelectorAll('link[rel="alternate"][hreflang]')).map(
		(el) => ({
	...
	const robotsText =
		...
		'None detected (default: index, follow)';
	...
	if (robotsLower.includes('noindex') || robotsLower.includes('none')) {
		...
		robotsMetaMessage =
			'Robots meta tag contains "noindex". This page will NOT appear in search engines.';
	} else if (robotsLower.includes('nofollow')) {
		robotsMetaStatus = 'warning';
		robotsMetaMessage =
			'Robots meta tag contains "nofollow". Search engines will not follow links on this page.';
	}
	...
	if (!favicon) {
		...
		faviconMessage =
			'Missing favicon. Search results look less professional without a branded icon.';
	}
	...
	const qaFormatting: GenericAudit = hasQA
		? {
				status: 'ok',
				message:
					'Page uses Question/Answer style headings which AI models favor for extracting facts.'
			}
		: {
				status: 'warning',
				message:
	...
	const scannability: GenericAudit =
		lists > 0 || tables > 0
			? {
					status: 'ok',
					message: `Found ${lists} lists and ${tables} tables. These structured elements are highly preferred by AI models for summarization.`
				}
			: {
					status: 'warning',
					message:
	...
	const semanticHtml: GenericAudit =
		semantics > 0
			? {
					status: 'ok',
					message: `Found ${semantics} semantic HTML5 elements. This helps AI build an accurate knowledge graph of your page.`
				}
			: {
					status: 'warning',
					message:
	...
	const hasTargetSchema = schemaTypes.some((t) =>
		['faqpage', 'article', 'newsarticle', 'organization', 'product', 'how-to', 'howto'].includes(
			t.toLowerCase()
		)
	);
	const targetSchema: GenericAudit = hasTargetSchema
		? {
				status: 'ok',
				message:
					'Detected high-value AEO schemas (e.g. Article, FAQ, Product, Organization) that AI engines actively consume.'
			}
		: {
				status: 'warning',
...
export async function fetchPageSpeed(
	...
	if (!res.ok) {
		...
		if (res.status === 429) {
			throw new Error(
				`PageSpeed API rate limit exceeded (429). Please provide an API Key in settings.`
			);
		} else if (res.status === 400 && !apiKey) {
			throw new Error(
				`PageSpeed API request failed (400). An API Key may be required for this domain.`
			);
		}
