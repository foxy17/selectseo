<script lang="ts">
  ...
  let aiStatus = $state<'unknown' | 'available' | 'downloadable' | 'downloading' | 'unavailable' | 'unsupported'>('unknown');
...
Suggest actionable improvements to structure the content layout and headings to target Google's Featured Snippets, simplify any jargon, and optimize readability.`
  ...
  function detectBrowser() {
    ...
    const hasChrome = (ua.includes('Chrome') || ua.includes('CriOS')) && vendor.includes('Google Inc.');
...
Your goal is to answer the user's questions about this SEO audit, provide actionable advice, or generate optimized content (like rewriting titles/descriptions). Format your responses in markdown.`;
      ...
      messages.push({
        role: 'assistant',
        text: 'Hello! I am your local Gemini Nano Copilot. I have analyzed your SEO audit for **' + auditResults.url + '**. What would you like to know or optimize?'
      });
    } catch (e) {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        ...
        console.error(err);
        messages[messages.length - 1].text = `[ERROR] Prompt execution aborted: ${err.message || err}`;
        messages = [...messages];
      }
    } finally {
      isGenerating = false;
...
<div class="ai-chat-tab">
  <div class="chat-header card-dark">
    <div class="header-left">
      ...
      {#if aiStatus === 'available'}
        <button class="new-chat-btn font-mono" onclick={startNewChat} title="Start a new chat session">
          <svg class="new-chat-icon" xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
          New Chat
        </button>
      {/if}
    </div>
    ...
    <div class="chat-container card-dark mt-4">
      <div class="chat-history" class:welcome-state={messages.length <= 1} bind:this={chatBoxEl}>
        ...
        {:else if messages.length === 1}
          ...
          <div class="welcome-hero animate-fade-in">
            ...
            <p class="hero-desc">
              I have analyzed the SEO audit for <code class="hero-url">{auditResults.url}</code>. Select a quick action below or write a custom query to start optimizing your search performance.
            </p>
      ...
      <div class="chat-input-area-container">
        ...
        <div class="chat-input-area">
          ...
          ></textarea>
          {#if isGenerating}
            <button class="btn send-btn stop-btn" onclick={stopGeneration}>
              Stop
            </button>
          {:else}
  ...
  {:else if !isChrome && !overrideBrowserCheck}
    ...
    <div class="setup-guide-box card-dark mt-4 border-error">
      ...
      <p class="text-muted mt-3">
        The SEO AI Copilot uses <strong>Google Chrome's native Prompt API</strong> to run a local instance of <strong>Gemini Nano</strong> directly in your browser. 
      </p>
      <p class="text-muted mt-2">
        Running AI models locally ensures your crawled site data is analyzed with <strong>100% privacy</strong> (nothing is sent to external servers) and runs entirely offline.
      </p>
      ...
      <div class="download-section mt-4 font-mono">
        ...
        <div class="action-buttons mt-3 flex gap-3">
          <a href="https://www.google.com/chrome/" target="_blank" rel="noopener noreferrer" class="btn btn-primary text-center">
      ...
      <div class="browser-info mt-4 font-mono text-xxs text-muted">
        <span class="label">Detected User Agent:</span> <span class="val text-body">{typeof navigator !== 'undefined' ? navigator.userAgent : 'Unknown'}</span>
      </div>
    </div>
  ...
  {:else}
    ...
    <div class="setup-guide-box card-dark mt-4 border-warning">
      ...
      <p class="text-muted mt-3 text-sm">
        Google Chrome is detected, but its built-in local AI model (Gemini Nano) is either disabled or not yet downloaded. Follow these steps to configure your browser:
      </p>
      ...
      <div class="steps-container mt-4">
        ...
        <div class="step-card">
          ...
          <div class="step-content">
            ...
            <span class="text-xs text-muted">Find <strong class="text-primary">#prompt-api-for-gemini-nano</strong> and set it to:</span>
        ...
        <div class="step-card">
          ...
          <div class="step-content">
            ...
            <span class="text-xs text-muted">Find <strong class="text-primary">#optimization-guide-on-device-model</strong> and set it to:</span>
        ...
        <div class="step-card">
          ...
          <div class="step-content">
            ...
            <span class="text-xs text-muted">Click the <strong class="text-warning">Relaunch</strong> button at the bottom of the flags page to apply changes.</span>
          </div>
        </div>
        ...
        <div class="step-card font-mono">
          ...
          <div class="step-content">
            ...
            {#if aiStatus === 'downloadable'}
              <span class="text-xs text-muted font-sans">The model is ready to download! Click below to start the background download:</span>
              <div class="mt-2">
                <button class="btn btn-primary btn-sm" onclick={triggerDownload}>Download Model</button>
              </div>
            {:else}
              <span class="text-xs text-muted font-sans">If the Prompt API is still unavailable after relaunch, open <code class="code-link font-mono">chrome://components</code> and check for updates under <strong>Optimization Guide On Device Model</strong>. Then, refresh this page.</span>
            {/if}
          </div>
        </div>
      </div>
      ...
      <div class="status-footer mt-4 font-mono text-xs flex justify-between items-center">
        <span>Current AI Status: <strong class="text-warning">{aiStatus.toUpperCase()}</strong></span>
        {#if overrideBrowserCheck}
          <button class="btn-text text-xxs font-mono text-muted hover-primary" onclick={() => { overrideBrowserCheck = false; detectBrowser(); }}>
...
<style>
  ...
  .status-indicator.online { color: var(--color-success); text-shadow: 0 0 8px var(--color-success); }
  .status-indicator.warning { color: var(--color-warning); text-shadow: 0 0 8px var(--color-warning); }
  .status-indicator.offline { color: var(--color-muted); }
