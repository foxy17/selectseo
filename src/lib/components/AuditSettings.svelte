  ...
  <div class="settings-grid">
    <div class="field proxy-field-group">
      <div class="proxy-select-wrap">
        ...
        <select id="proxy-select" class="text-input select-input" bind:value={settings.selectedProxy}>
      ...
      {#if settings.selectedProxy === 'custom'}
        <div class="proxy-input-wrap animate-fade-in mt-1">
          ...
          <input id="proxy-input" type="text" class="text-input" bind:value={settings.customProxy} placeholder="e.g. https://myproxy.com/?url=" />
        </div>
      {/if}
    </div>
    <div class="field">
      <label for="pagespeed-input">PageSpeed API Key (Optional)</label>
      <input id="pagespeed-input" type="password" class="text-input" bind:value={settings.apiKey} placeholder="Google Cloud API Key" />
    </div>
