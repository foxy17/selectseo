<script lang="ts">
	...
	const headingWarnings = $derived.by(() => {
		...
		if (h1Count === 0)
			warnings.push(
				'Missing H1 heading: A page should always have exactly one H1 tag representing its primary topic.'
			);
		if (h1Count > 1)
			warnings.push(
				`Multiple H1 headings: Found ${h1Count} H1 tags. It is recommended to keep exactly one H1 per page.`
			);
		if (h3Count > 0 && h2Count === 0)
			warnings.push(
				'Skip-level H3: Found H3 headings but no H2 headings. Maintain a proper hierarchy (H1 -> H2 -> H3).'
			);
		if (h4Count > 0 && h3Count === 0)
			warnings.push('Skip-level H4: Found H4 headings but no H3 headings.');
		if (h5Count > 0 && h4Count === 0)
			warnings.push('Skip-level H5: Found H5 headings but no H4 headings.');
		if (h6Count > 0 && h5Count === 0)
			warnings.push('Skip-level H6: Found H6 headings but no H5 headings.');
...
<div class="on-page-tab">
		...
		<div class="preview-card card-dark" id="serp-preview-card">
			...
			<div class="serp-box" class:serp-mobile={serpMode === 'mobile'}>
				<div class="serp-url font-sans">
					{domainName} <span class="url-arrow">›</span>
					{auditResults.url.split('/').filter(Boolean).slice(2).join(' › ') || ''}
				</div>
				...
				<div class="serp-desc">
					...
					{:else}
						{auditResults.onPage.description.text ||
							'Please provide a meta description. Otherwise, search engines will construct a snippet from page contents.'}
					{/if}
				</div>
			</div>
			...
			<div class="preview-validations mt-2">
				{#if titleLength > 60}
					<div class="validation-item text-warning">
						⚠ Title exceeds optimal length ({titleLength}/60 chars) and will be truncated.
					</div>
				{:else if titleLength > 0 && titleLength < 30}
					<div class="validation-item text-muted">
						⚠ Title is shorter than recommended ({titleLength}/30-60 chars).
					</div>
				{/if}

				{#if descLength > 160}
					<div class="validation-item text-warning">
						⚠ Description exceeds optimal length ({descLength}/160 chars) and will be truncated.
					</div>
				{:else if descLength > 0 && descLength < 110}
					<div class="validation-item text-muted">
						⚠ Description is shorter than recommended ({descLength}/110-160 chars).
					</div>
				{/if}
			</div>
		</div>
			...
			{#if socialMode === 'facebook'}
				<div class="facebook-box">
					...
					<div class="fb-info">
						...
						<div class="fb-title">
							{auditResults.onPage.openGraph.title ||
								auditResults.onPage.title.text ||
								'Share Title'}
						</div>
						<div class="fb-desc">
							{auditResults.onPage.openGraph.description ||
								auditResults.onPage.description.text ||
								'Share description details...'}
						</div>
					</div>
				</div>
			{:else}
				<div class="twitter-box">
					...
					<div class="tw-info">
						...
						<div class="tw-title">
							{auditResults.onPage.twitterCard.title ||
								auditResults.onPage.title.text ||
								'Twitter Share Title'}
						</div>
						<div class="tw-desc">
							{auditResults.onPage.twitterCard.description ||
								auditResults.onPage.description.text ||
								'Twitter share summary description...'}
						</div>
					</div>
				</div>
			{/if}
	...
	<div class="gauges-card card-dark mt-4">
		...
		<div class="gauge-group mt-4" id="meta-desc-card">
			<div class="gauge-label-row">
				<span
					>Meta Description Length: <strong class="text-primary">{descLength}</strong> characters</span
				>
			...
			<ul class="meta-checklist mt-2 font-mono">
				<li class="checklist-row">
					...
					{#if auditResults.onPage.viewportAudit.status === 'ok'}
						<span class="text-success" title={auditResults.onPage.viewport}
							>{auditResults.onPage.viewportAudit.status.toUpperCase()}</span
						>
					{:else}
						<span class="text-error" title={auditResults.onPage.viewportAudit.message}
							>{auditResults.onPage.viewportAudit.status.toUpperCase()}</span
						>
					{/if}
				</li>
				...
				<li class="checklist-row">
					...
					{#if auditResults.onPage.languageAudit.status === 'ok'}
						<span class="text-success"
							>{auditResults.onPage.contentMetrics.language.toUpperCase()}</span
						>
					{:else}
				...
				<li class="checklist-row">
					...
					{:else}
						<span class="text-warning" title={auditResults.onPage.robotsMetaAudit.message}
							>{auditResults.onPage.robotsMetaAudit.status.toUpperCase()}</span
						>
					{/if}
				</li>
				<li class="checklist-row">
					...
					{:else}
						<span class="text-warning" title={auditResults.onPage.faviconAudit.message}
							>MISSING</span
						>
					{/if}
				</li>
	...
	<div class="schema-card card-dark mt-4" id="schema-card">
		<h3 class="title-md">
			Schema Structured Data ({auditResults.onPage.schemaTypes.length} found)
		</h3>
		<p class="section-desc mt-1">
			Structured JSON-LD schema helps search engines understand details about WebPages, Companies,
			Products, and rich snippets.
		</p>

		{#if auditResults.onPage.schemaTypes.length === 0}
			<div class="empty-state text-warning mt-2 font-mono">
				⚠ No JSON-LD structured schemas detected on this page. Search engines won't represent rich
		...
		{:else}
			<div class="schema-cards-container mt-2">
				{#each schemaItems as schema, i (i)}
					<Collapsible
						cardClass="schema-collapsible-card"
						headerClass="schema-header-btn"
						bodyClass="schema-json-body font-mono"
					>
						{#snippet header()}
...
<style>
	...
	.h2-badge {
		background-color: rgb(var(--color-accent-blue-rgb) / 0.15);
