<script lang="ts">
	...
	const categoryScores = $derived.by(() => {
		...
		if (auditResults.onPage.imageAlts.total > 0) {
			const missingPct =
				auditResults.onPage.imageAlts.missing / auditResults.onPage.imageAlts.total;
		...
		return {
			...
			performance:
				perfScore !== null ? { score: perfScore, grade: calculateGrade(perfScore) } : null
		};
	});
		...
		const issues = [
			{
				...
				whyItMatters:
					'The title tag is the single most important on-page SEO element. It tells search engines what the page is about and is displayed in search results.',
				howToFix:
					'Ensure every page has a unique, descriptive title tag between 50-60 characters long.',
				impact: 'High',
				tab: 'onpage',
			...
			},
			{
				...
				whyItMatters:
					'Meta descriptions influence click-through rates from search engine results pages (SERPs) by summarizing the page content.',
				howToFix:
					'Write a compelling, unique meta description between 150-160 characters containing key search terms.',
				impact: 'Medium',
				tab: 'onpage',
			...
			},
			{
				...
				whyItMatters:
					'Canonical tags prevent duplicate content issues by specifying the primary (authoritative) version of a webpage to search engines.',
				howToFix:
					'Add a <link rel="canonical" href="..."/> tag pointing to the absolute, preferred URL of this page.',
				impact: 'High',
				tab: 'onpage',
			...
			},
			{
				...
				status:
					auditResults.onPage.headings.status === 'ok'
						? 'ok'
						: auditResults.onPage.headings.status === 'error'
							? 'error'
							: 'warning',
				whyItMatters:
					'An H1 heading represents the page title. Having exactly one H1 structures the page logically for users and search crawlers.',
			...
			},
			{
				...
				whyItMatters:
					'Alt tags describe images to visually impaired users and allow search engines to understand and index image content for image search.',
				howToFix:
					'Add descriptive, keyword-rich alt attributes to all <img> tags that convey content or context.',
				impact: 'Medium',
				tab: 'onpage',
			...
			},
			{
				...
				description:
					auditResults.links.filter((l) => l.statusState === 'broken').length > 0
				...
				status:
					auditResults.links.filter((l) => l.statusState === 'broken').length > 0 ? 'error' : 'ok',
				whyItMatters:
					'Broken links (404s) degrade user experience, waste crawl budget, and signal search engines that the website might be unmaintained.',
				howToFix:
					'Locate the broken URLs in the Links tab and update the href attribute or remove the invalid links.',
				impact: 'High',
				tab: 'links',
			...
			},
			{
				...
				description:
					auditResults.links.filter((l) => !l.isSecure).length > 0
				...
				whyItMatters:
					'Serving pages over HTTPS and linking securely prevents man-in-the-middle attacks and is a lightweight ranking signal.',
			...
			},
			{
				...
				description:
					auditResults.pageSpeedMobile || auditResults.pageSpeedDesktop
				...
				status:
					auditResults.pageSpeedMobile || auditResults.pageSpeedDesktop
						? (auditResults.pageSpeedMobile?.score ?? 100) < 50 ||
							(auditResults.pageSpeedDesktop?.score ?? 100) < 50
							? 'error'
							: (auditResults.pageSpeedMobile?.score ?? 100) < 90 ||
								  (auditResults.pageSpeedDesktop?.score ?? 100) < 90
								? 'warning'
								: 'ok'
						: 'warning',
				whyItMatters:
					'Page speed, Core Web Vitals (LCP, FID/INP, CLS) are official Google ranking signals representing page experience.',
				howToFix:
					'Execute a PageSpeed audit on the PageSpeed tab and implement the recommendations like minification and image sizing.',
				impact: 'High',
				tab: 'pagespeed',
		...
		<div class="category-breakdown-grid grid grid-3">
			<div class="category-card card-dark">
				<div class="card-header">
					...
					<span
						...
						class:text-success={scoreBand(categoryScores.meta.score) === 'success'}
						class:text-warning={scoreBand(categoryScores.meta.score) === 'warning'}
						class:text-error={scoreBand(categoryScores.meta.score) === 'error'}
						>{categoryScores.meta.grade}</span
					>
				</div>
			...
			<div class="category-card card-dark">
				<div class="card-header">
					...
					<span
						class="category-grade"
						class:text-success={scoreBand(categoryScores.structure.score) === 'success'}
						class:text-warning={scoreBand(categoryScores.structure.score) === 'warning'}
						class:text-error={scoreBand(categoryScores.structure.score) === 'error'}
						>{categoryScores.structure.grade}</span
					>
				</div>
			...
			<div class="category-card card-dark">
				<div class="card-header">
					...
					<span
						class="category-grade"
						class:text-success={scoreBand(categoryScores.media.score) === 'success'}
						class:text-warning={scoreBand(categoryScores.media.score) === 'warning'}
						class:text-error={scoreBand(categoryScores.media.score) === 'error'}
						>{categoryScores.media.grade}</span
					>
				</div>
			...
			<div class="category-card card-dark">
				<div class="card-header">
					...
					<span
						class="category-grade"
						class:text-success={scoreBand(categoryScores.links.score) === 'success'}
						class:text-warning={scoreBand(categoryScores.links.score) === 'warning'}
						class:text-error={scoreBand(categoryScores.links.score) === 'error'}
						>{categoryScores.links.grade}</span
					>
				</div>
			...
			<div class="category-card card-dark">
				<div class="card-header">
					...
					{#if categoryScores.performance}
						<span
							class="category-grade"
							class:text-success={scoreBand(categoryScores.performance.score) === 'success'}
							class:text-warning={scoreBand(categoryScores.performance.score) === 'warning'}
							class:text-error={scoreBand(categoryScores.performance.score) === 'error'}
							>{categoryScores.performance.grade}</span
						>
					{:else}
				...
				<ProgressBar
					value={categoryScores.performance?.score ?? 0}
					band={categoryScores.performance?.score}
				/>
				{#if categoryScores.performance}
			...
			>
				<div class="issue-head">
					<div class="issue-head-left">
						<span
							class="badge"
							class:badge-success={issue.status === 'ok'}
							class:badge-warning={issue.status === 'warning'}
							class:badge-error={issue.status === 'error' || issue.status === 'missing'}
						>
							{issue.status.toUpperCase()}
						</span>
						<span
							class="badge badge-pill impact-badge"
							class:impact-high={issue.impact === 'High'}
							class:impact-medium={issue.impact === 'Medium'}
						>
							{issue.impact} Impact
						</span>
