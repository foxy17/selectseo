...
<div class="pagespeed-tab" id="pagespeed-section">
	...
	{#if !auditResults.pageSpeedMobile && !auditResults.pageSpeedDesktop && !isFetchingDesktopSpeed && !isFetchingMobileSpeed}
		<div class="trigger-audit-card card-dark text-center font-sans">
			...
			<p class="mt-2 text-muted max-w-sm">
				Connects to the Google PageSpeed Insights API to pull comprehensive Lighthouse performance
				metrics, loading times, and diagnostics for Mobile & Desktop strategy views.
			</p>
			<button class="btn btn-primary mt-4" onclick={() => triggerPageSpeedAudits(auditResults.url)}
				>Run Performance Audit</button
			>
		</div>
	{/if}
	...
	{#if isFetchingDesktopSpeed || isFetchingMobileSpeed}
		<div class="trigger-audit-card card-dark text-center font-mono border-warning">
			...
			<p class="text-muted mt-2">
				Running Lighthouse visual performance simulations. This may take up to 20-30 seconds.
			</p>
		</div>
	{/if}
	...
	{#if (auditResults.pageSpeedMobile || auditResults.pageSpeedDesktop) && !isFetchingDesktopSpeed && !isFetchingMobileSpeed}
			...
			<div class="explainer-panel-box card-dark border-primary mt-4 font-sans">
				{#if activeExplainer === 'lcp'}
					...
					<p class="mt-1 text-muted">
						LCP measures when the largest content element (typically a hero image, video banner, or
						large heading block) is rendered onto the viewport screen. To optimize LCP: compression
						of images, preloading LCP resources, and removing render-blocking styles/scripts.
					</p>
				{:else if activeExplainer === 'tbt'}
					<h4 class="title-sm text-primary">Total Blocking Time (TBT)</h4>
					<p class="mt-1 text-muted">
						TBT calculates the total milliseconds of duration where client browser responsiveness is
						blocked by long-executing scripts (blocking main-thread tasks exceeding 50ms). Optimize
						TBT by splitting JS tasks, reducing bundle sizes, and deferring non-essential scripts.
					</p>
				{:else if activeExplainer === 'cls'}
					<h4 class="title-sm text-primary">Cumulative Layout Shift (CLS)</h4>
					<p class="mt-1 text-muted">
						CLS measures how much elements visually jump around on the screen during the loading
						cycle. Optimize CLS by ensuring all images/media have explicit width/height dimensions,
						using CSS aspect-ratio, and avoiding loading dynamic layouts above the fold.
					</p>
				{/if}
			</div>
		{/if}
		...
		<div class="recommendations-container mt-4 font-sans">
			...
			{#if auditResults.pageSpeedMobile?.recommendations && auditResults.pageSpeedMobile.recommendations.length > 0}
				<div class="rec-list mt-2">
					{#each auditResults.pageSpeedMobile.recommendations as rec (rec.title)}
						<Collapsible
							cardClass="rec-card card-dark collapsible-rec"
							headerClass="rec-header-btn"
							bodyClass="rec-body"
							arrowSpaced
						>
							{#snippet header()}
							...
							<p class="rec-learn-more mt-2">
								<a href="https://web.dev/fast/" target="_blank" rel="noreferrer"
									>Learn how to audit this recommendation on Web.dev →</a
								>
							</p>
						</Collapsible>
					{/each}
			...
			{:else}
				<p class="empty-state text-success mt-2">
					All performance optimization criteria have passed. Perfect!
				</p>
			{/if}
		</div>
		...
		{#if auditResults.pageSpeedMobile?.passedAudits && auditResults.pageSpeedMobile.passedAudits.length > 0}
			<div class="passed-audits-section mt-4 font-sans">
				<button
					class="passed-header-btn font-mono"
					onclick={() => (showPassedAudits = !showPassedAudits)}
				>
