<script lang="ts">
  ...
  let queryHistory = $state<Array<{ query: string; timestamp: string; rowsCount: number; error: boolean }>>([]);
  ...
  function copyMarkdown() {
    ...
    sqlResult.rows.forEach((row: any) => {
      md += '| ' + sqlResult.columns.map((col: string) => {
        const val = row[col] === null ? 'NULL' : row[col];
      ...
      <div class="schema-sidebar card-dark font-mono">
        ...
        <div class="table-tree mt-2">
          ...
          <div class="table-node">
            <button class="node-title-btn" onclick={() => insertText('SELECT * FROM meta LIMIT 10;')}>
          ...
          <div class="table-node mt-2">
            <button class="node-title-btn" onclick={() => insertText('SELECT * FROM headings LIMIT 10;')}>
              📁 headings <span class="badge badge-pill text-xs">{auditResults.onPage.headings.h1.length + auditResults.onPage.headings.h2.length + auditResults.onPage.headings.h3.length} rows</span>
            </button>
            <div class="columns-list">
          ...
          <!-- Table: links -->
          <div class="table-node mt-2">
            <button class="node-title-btn" onclick={() => insertText('SELECT * FROM links LIMIT 10;')}>
          ...
          <!-- Table: images -->
          <div class="table-node mt-2">
            <button class="node-title-btn" onclick={() => insertText('SELECT * FROM images LIMIT 10;')}>
              📁 images <span class="badge badge-pill text-xs">{auditResults.onPage.imageAlts.total} rows</span>
            </button>
            <div class="columns-list">
              <button onclick={() => insertText('id')}>id (integer)</button>
    ...
    <div class="workspace-main flex-1">
      ...
      <div class="toolbar-row font-mono mb-2">
        <div class="toolbar-left">
          ...
          <button class="preset-pill" onclick={() => setSampleQuery("SELECT href, text, status FROM links WHERE status >= 400;")}>Broken Links</button>
          <button class="preset-pill" onclick={() => setSampleQuery("SELECT src, alt FROM images WHERE is_missing = 1;")}>Missing Alt</button>
          <button class="preset-pill" onclick={() => setSampleQuery("SELECT tag, COUNT(*) as count FROM headings GROUP BY tag;")}>Headings count</button>
          <button class="preset-pill" onclick={() => setSampleQuery("SELECT type, COUNT(*), SUM(secure) FROM links GROUP BY type;")}>Secure links</button>
        </div>
        <div class="toolbar-right">
          <button class="tool-btn" onclick={() => showSchema = !showSchema}>{showSchema ? 'Hide Schema' : 'Show Schema'}</button>
          <button class="tool-btn" onclick={() => showHistory = !showHistory}>{showHistory ? 'Hide History' : 'Query History'}</button>
        </div>
      </div>
      ...
      <div class="exec-row mt-2 font-mono">
        <span class="exec-tip text-muted">Press <strong class="text-primary">Ctrl + Enter</strong> to execute</span>
      ...
      {#if sqlError}
        <div class="sql-error-block mt-2 font-mono">
          <span class="error-prefix text-error">DB_EXCEPTION:</span> {sqlError}
        </div>
      {/if}
            ...
            {:else}
              {#each queryHistory as hist}
                <div class="history-item">
                  <button class="hist-query-text text-left-align" onclick={() => setSampleQuery(hist.query)}>{hist.query}</button>
              ...
              <table class="data-table sql-data-table">
                  ...
                  <tr>
                    {#each sqlResult.columns as col}
                      <th class="sortable-results-th" onclick={() => handleSortResults(col)}>
                        {col} {resultsSortCol === col ? (resultsSortDir === 'asc' ? '▲' : '▼') : ''}
                      </th>
                    {/each}
                  </tr>
                </thead>
                <tbody>
                  {#each sortedRows as row}
                    <tr>
                      {#each sqlResult.columns as col}
                        <td class="sql-cell" title={row[col]}>{row[col] === null ? 'NULL' : row[col]}</td>
                      {/each}
                    </tr>
                  {/each}
                </tbody>
