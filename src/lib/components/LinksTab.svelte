<script lang="ts">
	...
	let activeFilter = $state<
		'all' | 'internal' | 'external' | 'broken' | 'redirects' | 'insecure' | 'nofollow'
	>('all');
	let searchQuery = $state('');
	let sortColumn = $state<'href' | 'text' | 'type' | 'secure' | 'status' | 'responseTime'>(
		'status'
	);
	...
	const redirects = $derived(
		auditResults.links.filter((l) => l.status && l.status >= 300 && l.status < 400).length
	);
	const insecure = $derived(auditResults.links.filter((l) => !l.isSecure).length);
	const nofollow = $derived(
		auditResults.links.filter((l) => l.rel.toLowerCase().includes('nofollow')).length
	);
	...
	const filteredLinks = $derived.by(() => {
		...
		else if (activeFilter === 'redirects')
			items = items.filter((l) => l.status && l.status >= 300 && l.status < 400);
		else if (activeFilter === 'insecure') items = items.filter((l) => !l.isSecure);
		else if (activeFilter === 'nofollow')
			items = items.filter((l) => l.rel.toLowerCase().includes('nofollow'));
	...
	async function copyBrokenLinks() {
		const brokenUrls = auditResults.links
			.filter((l) => l.statusState === 'broken')
			.map((l) => l.href)
			.join('\n');
...
<div class="links-tab" id="links-auditor-section">
	...
	<div class="links-summary-ribbon card-dark">
		...
		<div class="dist-track mt-4">
			{#if total > 0}
				<div
					class="dist-segment segment-int"
					style="width: {intPct}%"
					title="Internal ({intPct.toFixed(1)}%)"
				></div>
				<div
					class="dist-segment segment-ext"
					style="width: {extPct}%"
					title="External ({extPct.toFixed(1)}%)"
				></div>
				<div
					class="dist-segment segment-red"
					style="width: {redPct}%"
					title="Redirects ({redPct.toFixed(1)}%)"
				></div>
				<div
					class="dist-segment segment-brk"
					style="width: {brkPct}%"
					title="Broken ({brkPct.toFixed(1)}%)"
				></div>
			{:else}
		...
		<div class="links-progress-banner card-dark border-warning mt-4">
			<div class="progress-details">
				...
				<span class="progress-ratio font-mono"
					>{checkedLinksCount} / {totalLinksCount} inspected</span
				>
			</div>
			<ProgressBar value={(checkedLinksCount / totalLinksCount) * 100} transition="0.2s" />
			<p class="progress-tip font-mono text-muted">
				Scanning pages in background... Checked URLs are updating in real-time.
			</p>
		</div>
	{/if}
	...
	<div class="bulk-actions mt-2 font-mono">
		...
		{#if broken > 0}
			<button class="btn btn-primary btn-sm btn-error-bg" onclick={copyBrokenLinks}
				>Copy Broken URLs</button
			>
		{/if}
		{#if copyStatus}
			<span
				class="copy-status"
				class:copy-status-error={copyStatusError}
				role="status"
					...
					{#each filteredLinks as link (link.id)}
						...
						>
							...
							<td>
								<span
									class="badge"
									class:badge-pill={!link.isExternal}
									class:badge-yellow={link.isExternal}
								>
											...
											<p class="detail-val">
												...
												{#if link.isExternal && !link.rel.includes('noopener')}
													<span
														...
														title="Security best practice: add rel='noopener' to external targets."
														>Missing noopener</span
													>
												{/if}
											</p>
										</div>
...
<style>
	...
	.pill-warning {
		background-color: rgb(var(--color-warning-rgb) / 0.12);
	...
	.pill-info {
		background-color: rgb(var(--color-accent-blue-rgb) / 0.12);
		color: var(--color-accent-blue);
		border: 1px solid rgb(var(--color-accent-blue-rgb) / 0.2);
	}
	.pill-info:hover {
		background-color: rgb(var(--color-accent-blue-rgb) / 0.25);
	}
